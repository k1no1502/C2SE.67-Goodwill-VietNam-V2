...
// PhpSpreadsheet 5.x requires newer PHP; fallback to legacy exporter when unavailable.
if (version_compare(PHP_VERSION, '8.3.0', '<')) {
    header('Location: export_reports.php?' . $fallbackQuery);
    exit;
}
...
// Also place both charts on the first sheet for quick visibility.
$overviewDonationSeries = clone $donationSeries;
$overviewDonationPlot = new PlotArea(null, [$overviewDonationSeries]);
$overviewDonationChart = new Chart(
    'overview_donation_trend',
    new Title('Thong ke quyen gop theo thang'),
    new Legend(Legend::POSITION_BOTTOM, null, false),
    $overviewDonationPlot
);
$overviewDonationChart->setTopLeftPosition('D4');
$overviewDonationChart->setBottomRightPosition('L18');
$overview->addChart($overviewDonationChart);

$overviewCategorySeries = clone $categorySeries;
$overviewCategoryPlot = new PlotArea(null, [$overviewCategorySeries]);
$overviewCategoryChart = new Chart(
    'overview_category_donut',
    new Title('Phan bo danh muc'),
    new Legend(Legend::POSITION_RIGHT, null, false),
    $overviewCategoryPlot
);
$overviewCategoryChart->setTopLeftPosition('D20');
$overviewCategoryChart->setBottomRightPosition('L34');
$overview->addChart($overviewCategoryChart);
...
$visualSheet->setTitle('Biểu đồ');
$reportMonthLabel = 'Bao cao theo thang ' . date('m/Y', strtotime($endDate));
$visualSheet->setCellValue('A1', 'BIỂU ĐỒ THỐNG KÊ');
$visualSheet->setCellValue('A2', $reportMonthLabel);
